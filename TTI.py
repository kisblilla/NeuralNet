...
train_df =pd.read_excel(
    'dataset.xlsx', sheet_name='TTI',na_values='?',  usecols=[
        ...
        'Aromás arány [%]','LOI [V/V%]','UL94','Alkalmazott hőáram [kW/m2]',
         'MLC TTI [s]'])
...
model = Sequential()
model.add(Dense(14, input_dim=16, kernel_initializer='normal', activation='selu'))
model.add(Dense(3, activation='selu'))
model.add(Dense(1, activation='selu'))
model.add(Dense(1, activation='linear'))
model.summary()

model.compile(loss='mae',optimizer = tf.keras.optimizers.RMSprop(learning_rate=0.0004),  metrics=['mse','mae'])
history=model.fit(xtrain_scale, ytrain_scale, epochs=300, batch_size=50, verbose=1, validation_split=0.2)
...
plt.xlabel('Validáció [TTI (s)]')
plt.ylabel('Predikció[TTI (s)]')
lims = [0, 40]
plt.xlim(lims)
plt.ylim(lims)
plt.plot(lims, lims)
...
plt.xlabel('Prediction Error [TTI]')
_ = plt.ylabel('Count')
